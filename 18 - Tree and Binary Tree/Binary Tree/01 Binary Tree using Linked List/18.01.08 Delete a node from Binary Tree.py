...
For deleting a node from a binary tree we need to built some helper functions:
    1. finding the deepest node using LevelOrder Traversal
    2. deleting the deepest node
    3. create a method to delete a node use these above methods:
        - first find the node
        - then replace the node value with the deepest value
        - now delete the deepest value
    by following these steps we can delete a node without moving any child nodes of that nodes
'''
class Node:
